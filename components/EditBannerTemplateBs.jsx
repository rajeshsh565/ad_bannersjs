"use client";
import { imgSuggestions } from "@/utils/bannersData";
import { motion } from "framer-motion";
import { useState } from "react";
import BannerImageComp from "./BannerImageComp";

const EditBannerTemplateBs = ({ editBannerProps, updateEditBanner }) => {
  const bannerInfo = editBannerProps?.bannerInfo;
  const setBannerInfo = editBannerProps?.handleBannerInfo;
  const [newImg, setNewImg] = useState(bannerInfo.img);
  const [newBgImg, setNewBgImg] = useState(bannerInfo.bgImg);
  const [newTitle, setNewTitle] = useState(bannerInfo.title);
  const [newDesc, setNewDesc] = useState(bannerInfo.desc);
  const [newCta, setNewCta] = useState(bannerInfo.cta);
  const [newCtaLabel, setNewCtaLabel] = useState(bannerInfo.ctaLabel);
  ...
  return (
    <motion.div
      ...
      className={`fixed top-0 left-0 w-full h-full bg-black flex justify-center items-center z-50 bg-[rgba(0,0,0,0.5)]`}
    >
      <div
      ...
      >
        ...
        >
          ...
          <input
            type="text"
            className="w-full h-10 border-1 px-2 rounded-md"
            defaultValue={newCtaLabel}
            onChange={(e) => {
              setNewCtaLabel(e.target.value);
